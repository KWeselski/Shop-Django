...
const cartReducer = (state = initState, action) => {
  ...
  if (action.type == DISCOUNT_NAMES.FAIL_DISCOUNT) {
    return { ...state, loading: false, error: action.error, discount: 0 };
  }
  if (action.type == DISCOUNT_NAMES.FINISH_DISCOUNT) {
    return { ...state, discount: action.payload.discount, loading: false };
  }
  ...
  if (action.type === REMOVE_ITEM) {
    ...
    if (itemToRemove.on_discount == true) {
      newTotal =
        Number(state.total) -
        Number(itemToRemove.discount_price * itemToRemove.quantity);
    } else {
      newTotal =
        Number(state.total) -
  ...
  if (action.type === ADD_QUANTITY) {
    ...
    if (addedItem.on_discount == true) {
      ...
      newTotal = Number(state.total) + Number(addedItem.price);
    }
    return {
      ...state,
      total: newTotal.toFixed(2)
