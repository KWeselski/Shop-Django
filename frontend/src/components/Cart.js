      ...
      <TableContainer>
        ...
        <TableBody>
          {this.props.items.map((item) => {
            // FIX: To sprawdzenie powinno być w SS
            let price = item.on_discount ? item.discount_price : item.price;
            return (
              <TableRow key={item.id}>
                ...
                <TableCell className={classes.tableCell} align="right">
                  {this.getTotal(price, item.quantity).toFixed(2)}$
                </TableCell>
                <TableCell component="th" scope="row">
                  <Link to="/cart">
                    <i
    ...
    return (
      <Grid container xs={12} md={12}>
        ...
        <Grid item xs={12} md={9}>
          <React.Fragment>{addedItems}</React.Fragment>
        </Grid>
          ...
          <Grid container xs={12} md={12}>
            ...
            <Grid item xs={12} md={12}>
              <Button
                ...
                component={Link}
                to={"/checkout"}
                disabled={!isAuthenticated || this.props.items.length < 1}
                variant="contained"
                color="primary"
                onClick={() => {
                  this.handleMakeOrder(this.props.items);
                }}
              >
                Make Order
              </Button>
